...
static INLINE_PROPERTY_REGEX: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r"\[([^\]]+)\]").unwrap()
});
...
// Grupo 2: separador (:: o :::)
static PROPERTY_PAIR_REGEX: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r"(\p{L}[\p{L}\p{N}_]*)\s*(:::?)\s*").unwrap()
});
...
impl InlinePropertyParser {
    ...
    pub fn parse(content: &str) -> Vec<InlineProperty> {
        ...
        for cap in INLINE_PROPERTY_REGEX.captures_iter(content) {
            ...
            let parsed = Self::parse_bracket_content(inner_content, line_number, char_start, char_end);
    ...
    fn parse_bracket_content(
        ...
        if pairs.len() == 1 {
            ...
            for i in 0..pairs.len() {
                let key_match = PROPERTY_PAIR_REGEX.captures(&escaped[pairs[i].start()..]).unwrap();
...
"#;
    ...
    fn test_escaped_comma() {
        ...
        assert!(matches!(&props[0].value, PropertyValue::Text(s) if s == "Cien años de soledad, novela"));
    }
