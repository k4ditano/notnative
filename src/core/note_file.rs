...
impl NoteFile {
    ...
    pub fn create<P: AsRef<Path>>(path: P, initial_content: &str) -> Result<Self> {
        ...
        if let Some(parent) = path.parent() {
            fs::create_dir_all(parent)
                .context("No se pudo crear el directorio padre")?;
        }
        
        // Escribir contenido inicial
        fs::write(&path, initial_content)
            .context("No se pudo escribir el archivo")?;
    ...
    pub fn read(&self) -> Result<String> {
        fs::read_to_string(&self.path)
            .context("No se pudo leer el archivo")
    }
...
impl NotesDirectory {
    ...
    pub fn new<P: AsRef<Path>>(root: P) -> Result<Self> {
        ...
        if !root.exists() {
            fs::create_dir_all(&root)
                .context("No se pudo crear el directorio de notas")?;
        }
    ...
    pub fn create_note_in_folder(&self, folder: &str, name: &str, content: &str) -> Result<NoteFile> {
        let filename = format!("{}.md", name);
...
mod tests {
    ...
    fn test_create_and_read_note() {
        ...
        let note = notes_dir.create_note("test", "# Test Note\n\nHello World").unwrap();
