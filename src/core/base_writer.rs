...
impl<'a> BaseWriter<'a> {
    ...
    pub fn update_property_value(
        ...
        for (_gid, char_start, char_end) in identical_groups {
            ...
            if char_end > content.len() || char_start >= char_end {
                eprintln!(
                    "⚠️ Posiciones inválidas para grupo: {}..{} en contenido de {} bytes",
                    char_start,
                    char_end,
                    content.len()
                );
            ...
            if !group_text.starts_with('[') || !group_text.ends_with(']') {
                eprintln!(
                    "⚠️ Grupo inválido en posición {}..{}: '{}'",
                    char_start, char_end, group_text
                );
                continue;
            }
    ...
    pub fn add_property_to_group(
        ...
        let (_line_num, char_start, char_end) = self
            .db
            .get_group_location(note_id, group_id)
            .map_err(|e| BaseWriterError::Database(e.to_string()))?
            .ok_or(BaseWriterError::GroupNotFound(note_id, group_id))?;
    ...
    fn replace_property_in_group(
        ...
        for prop in props.iter().rev() {
            // Iteramos en reversa para no afectar los índices
